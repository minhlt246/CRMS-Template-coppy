$(document).ready(function(){
    $("#btn-block-show-code").click(function(){
        $(".block-menu-left").fadeToggle("slow");
...
function showpass_2() {
  var x = document.getElementById("input_pass_2");
  if (x.type === "password") {
    x.type = "text";
  } else {
    x.type = "password";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch("https://restcountries.com/v3.1/all")
          ...
          countries.forEach(country => {
            if (country.idd && country.idd.root) {
              ...
              suffixes.forEach(suffix => {
                ...
                if (isdSelect) isdSelect.appendChild(option);
              });
      ...
      .catch(error => {
        ...
        if (isdSelect) isdSelect.appendChild(errorOption);
      });
  });
...
// XY Chart - chỉ khởi tạo khi DOM ready
document.addEventListener('DOMContentLoaded', function() {
    // Kiểm tra xem có element xyChart không
    const chartElement = document.getElementById('xyChart');
    if (chartElement) {
        google.charts.load('current', {'packages':['scatter']});
        google.charts.setOnLoadCallback(drawXYChart);
    }
});
